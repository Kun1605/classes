            ...
            table.bootstrapTable({
                ...
                columns: [
                    [
                        ...
                        //直接响应搜索
                        {field: 'login_name', title: __('用户名')},
                        {field: 'plain_password', title: __('密码')},
                        {field: 'password', title: __('铭文密码')},
                        {field: 'salt', title: __('盐')},
                        {field: 'email', title: __('邮箱')},
                        {field: 'status', title: __('状态')},
                        {field: 'token', title: __('令牌')},
                        {field: 'create_time', title: __('创建时间')},
                        {field: 'update_time', title: __('更新时间')},
...
/*                        //模糊搜索
                        {field: 'name', title: __('Title'), operate: 'LIKE %...%', placeholder: '模糊搜索，*表示任意字符', style: 'width:200px'},
                        //通过Ajax渲染searchList
                        {field: 'plain_password', title: __('Url'), align: 'left', searchList: $.getJSON('ajax/typeahead?search=a&field=row[user_id]'), formatter: Controller.api.formatter.url},
                        //点击IP时同时执行搜索此IP,同时普通搜索使用下拉列表的形式
                        {field: 'password', title: __('IP'), searchList: ['127.0.0.1', '127.0.0.2'], events: Controller.api.events.ip, formatter: Controller.api.formatter.ip},
                        //browser是一个不存在的字段
                        //通过formatter来渲染数据,同时为它添加上事件
                        {field: 'salt', title: __('Browser'), operate: false, events: Controller.api.events.browser, formatter: Controller.api.formatter.browser},
                        //启用时间段搜索
